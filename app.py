...
.proplens-logo { font-size: 2.6rem; font-weight: 800; letter-spacing: -2px; background: linear-gradient(135deg, #f97316 0%, #fb923c 50%, #fdba74 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; line-height: 1; }
.proplens-sub { font-family: 'DM Mono', monospace; font-size: 0.72rem; color: #64748b; letter-spacing: 0.15em; text-transform: uppercase; margin-top: 2px; }
.stat-card { background: linear-gradient(135deg, #0f172a 0%, #111827 100%); border: 1px solid #1e293b; border-radius: 14px; padding: 1.1rem 1.3rem; margin-bottom: 0.75rem; }
.stat-label { font-family: 'DM Mono', monospace; font-size: 0.65rem; color: #475569; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 4px; }
.stat-value { font-size: 1.8rem; font-weight: 800; color: #f1f5f9; letter-spacing: -1px; line-height: 1.1; }
.stat-value.orange { color: #f97316; } .stat-value.green { color: #22c55e; } .stat-value.red { color: #ef4444; } .stat-value.yellow { color: #eab308; }
.defense-card { background: linear-gradient(135deg, #0f172a 0%, #111827 100%); border: 1px solid #1e293b; border-radius: 14px; padding: 1rem 1.3rem; margin-bottom: 0.75rem; display: flex; align-items: center; justify-content: space-between; }
.defense-badge { font-family: 'DM Mono', monospace; font-size: 0.7rem; font-weight: 500; padding: 4px 12px; border-radius: 999px; letter-spacing: 0.05em; }
...
.verdict-banner { border-radius: 16px; padding: 1.5rem 2rem; margin: 1.5rem 0; border: 1px solid #1e293b; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; }
...
.verdict-tier.green { color: #22c55e; } .verdict-tier.yellow { color: #eab308; } .verdict-tier.orange { color: #f97316; } .verdict-tier.red { color: #ef4444; } .verdict-tier.gray { color: #64748b; }
.section-header { font-size: 0.65rem; font-family: 'DM Mono', monospace; letter-spacing: 0.2em; text-transform: uppercase; color: #f97316; margin: 1.75rem 0 0.75rem 0; padding-bottom: 6px; border-bottom: 1px solid #1e293b; }
.ai-box { background: linear-gradient(135deg, #0a0f1e 0%, #0d1525 100%); border: 1px solid #1e3a5f; border-radius: 14px; padding: 1.5rem; margin-top: 1rem; font-size: 0.95rem; line-height: 1.75; color: #cbd5e1; }
.model-note { background: #0f172a; border: 1px solid #1e293b; border-radius: 10px; padding: 0.75rem 1rem; margin-top: 0.5rem; font-family: 'DM Mono', monospace; font-size: 0.7rem; color: #475569; line-height: 1.6; }
...
.stButton > button { background: linear-gradient(135deg, #ea580c, #f97316) !important; color: white !important; border: none !important; border-radius: 8px !important; font-family: 'Syne', sans-serif !important; font-weight: 700 !important; letter-spacing: 0.03em !important; padding: 0.5rem 1.5rem !important; transition: opacity 0.2s !important; }
...
# ── Session state ──────────────────────────────────────────────────────────────
if "logs" not in st.session_state:          st.session_state.logs = None
if "ai_analysis" not in st.session_state:   st.session_state.ai_analysis = None
if "ai_error" not in st.session_state:      st.session_state.ai_error = None
if "defense_data" not in st.session_state:  st.session_state.defense_data = None
...
def get_last_n_games(player_id: int, season: str, n: int = 10) -> pd.DataFrame:
    df = playergamelog.PlayerGameLog(player_id=player_id, season=season, timeout=15).get_data_frames()[0]
...
def get_live_roster(team_abbr: str, season: str) -> list:
    ...
    try:
        df = commonteamroster.CommonTeamRoster(team_id=team_id, season=season, timeout=20).get_data_frames()[0]
...
def get_next_opponent(player_team: Optional[str]) -> Tuple[Optional[str], Optional[str], bool]:
    """Returns (opponent_abbr, game_date, is_back_to_back)."""
    if not player_team: return None, None, False
    from datetime import timedelta
    base_date = datetime.today()
    for day_offset in [0, 1, 2]:
        try:
            check_date = base_date + timedelta(days=day_offset)
            sb = scoreboardv2.ScoreboardV2(game_date=check_date.strftime("%m/%d/%Y"), league_id="00", day_offset=0, timeout=20)
            games = sb.get_data_frames()[0]
            if games.empty: continue
            row = games[(games["HOME_TEAM_ABBREVIATION"] == player_team) | (games["VISITOR_TEAM_ABBREVIATION"] == player_team)]
            if row.empty: continue
            row = row.iloc[0]
            home = row["HOME_TEAM_ABBREVIATION"]
            away = row["VISITOR_TEAM_ABBREVIATION"]
            game_date = check_date.strftime("%b %d, %Y")
            # Check back-to-back: did team play yesterday?
            is_b2b = False
            try:
                yesterday = check_date - timedelta(days=1)
                sb_prev = scoreboardv2.ScoreboardV2(game_date=yesterday.strftime("%m/%d/%Y"), league_id="00", day_offset=0, timeout=15)
                prev_games = sb_prev.get_data_frames()[0]
                if not prev_games.empty:
                    prev_row = prev_games[(prev_games["HOME_TEAM_ABBREVIATION"] == player_team) | (prev_games["VISITOR_TEAM_ABBREVIATION"] == player_team)]
                    is_b2b = not prev_row.empty
            except Exception: pass
            return (away, game_date, is_b2b) if player_team == home else (home, game_date, is_b2b)
    ...
    try:
        ...
        for player_name in roster:
            try:
                ...
                if not pid: continue
                log = playergamelog.PlayerGameLog(player_id=pid, season=season, timeout=15).get_data_frames()[0]
                if log.empty or "PLUS_MINUS" not in log.columns: continue
                pm = pd.to_numeric(log["PLUS_MINUS"], errors="coerce").dropna()
                if len(pm) < 5: continue
                all_pm.extend(pm.tolist())
                if len(all_pm) >= 20: break
            except Exception: continue
        if all_pm:
            opp_pts = round(avg_pts - sum(all_pm) / len(all_pm), 1)
    except Exception: pass
    if opp_pts is None: return "Neutral", None, avg_str
    diff = opp_pts - avg_pts
    if diff >= 1.5: return "Good", opp_pts, avg_str
    if diff <= -1.5: return "Bad", opp_pts, avg_str
...
def consistency_score(df: pd.DataFrame, line: float) -> float:
    ...
    return float((abs(pts - line) <= 3).sum() / len(pts))
...
def home_away_split(df: pd.DataFrame, line: float, side: str, player_team: Optional[str]) -> dict:
    ...
    if df is None or df.empty or "MATCHUP" not in df.columns: return result
    df = df.copy()
...
def blend_venue_rate(weighted_base: float, splits: dict, tonight_venue: Optional[str]) -> Tuple[float, bool]:
    """
    Blend venue-specific hit rate into overall weighted rate.
    Only applies when tonight's venue is known AND the split has 4+ games.
    Blend: 60% overall + 40% venue-specific.
    Returns (blended_rate, was_applied).
    """
    if not tonight_venue or not splits:
        return weighted_base, False
    venue_rate  = splits.get("home_rate")  if tonight_venue == "Home" else splits.get("away_rate")
    venue_games = splits.get("home_games") if tonight_venue == "Home" else splits.get("away_games")
    if venue_rate is None or (venue_games or 0) < 4:
        return weighted_base, False
    return round(0.60 * weighted_base + 0.40 * venue_rate, 4), True

def trend_flag(series: pd.Series, n: int) -> str:
    s = pd.to_numeric(series, errors="coerce").dropna()
    lookback = max(2, n // 3)
    if len(s) < lookback + 2: return "nodata"
    diff = s.iloc[:lookback].mean() - s.iloc[lookback:].mean()
...
def suggest_bucket(value: float, strong_cut: float, risk_cut: float) -> str:
    if value >= strong_cut: return "Strong"
    if value < risk_cut:    return "Risk"
    return "Okay"

def apply_adjustments(base_rate: float, context: dict) -> float:
    multipliers = {
...
def get_confidence_tier(adjusted: float, line_diff: float, consistency: float) -> str:
    ...
    if adjusted >= 0.64 and line_diff >= 1.5:   tier = "Strong Over"
    elif adjusted >= 0.55 and line_diff > 0:     tier = "Lean Over"
    elif adjusted <= 0.36 and line_diff <= -1.5: tier = "Strong Under"
    elif adjusted <= 0.45 and line_diff < 0:     tier = "Lean Under"
    else:                                         tier = "Pass"
    if low_consistency:
        if tier == "Strong Over":  tier = "Lean Over"
        if tier == "Strong Under": tier = "Lean Under"
    return tier
...
def build_points_chart(logs: pd.DataFrame, full_name: str, line: float, avg_pts: float) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(pts))), y=pts, mode="lines+markers",
        line=dict(color="#60a5fa", width=2.5),
        marker=dict(color=colors, size=11, line=dict(color="#080c14", width=2)),
        hovertemplate=[f"<b>{labels.iloc[i]}</b><br>Points: <b>{pts[i]}</b><br>{'✅ Over' if pts[i] >= line else '❌ Under'}<extra></extra>" for i in range(len(pts))],
    ))
    ...
    fig.update_layout(
        xaxis=dict(tickmode="array", tickvals=list(range(len(pts))), ticktext=[labels.iloc[i] for i in range(len(pts))],
                   tickangle=-30, showgrid=False, tickfont=dict(size=10, color="#475569"), linecolor="#1e293b"),
        yaxis=dict(title="PTS", showgrid=True, gridcolor="rgba(30,41,59,0.8)", tickfont=dict(size=10, color="#475569")),
        plot_bgcolor="#080c14", paper_bgcolor="#080c14", font=dict(color="#e2e8f0"),
        hovermode="x unified", hoverlabel=dict(bgcolor="#0f172a", bordercolor="#1e293b"),
...
def build_analysis_prompt(
    ...
    baseline, weighted_base, venue_blended, adjusted, tier,
    ...
    venue_note = "Venue split applied (40% weight)" if venue_applied else "Venue split not applied (< 4 games in split)"
...
- Venue-blended rate: {venue_blended:.0%} | Final adjusted rate: {adjusted:.0%}
- Consistency: {consistency:.0%} | {venue_note}
...
Write 3-4 paragraphs: (1) lead with the prop and lean, (2) what the game log shows — mention home/away split if meaningful, (3) how tonight's venue and opponent defense affect the prop, (4) closing verdict. Be direct, use real numbers, write like a sharp bettor."""
...
def get_all_player_names():
    all_players = players.get_players()
    return sorted([p["full_name"] for p in all_players if p.get("is_active", True)], key=lambda x: x.split()[-1])

all_player_names = get_all_player_names()

col_a, col_b, col_c, col_d, col_e = st.columns([2.5, 1, 1, 1, 0.8])
with col_a:
    selected_player = st.selectbox("Player", options=[None] + all_player_names, index=0,
        format_func=lambda x: "🔍  Type to search for a player..." if x is None else x,
        help="Start typing a player's name to filter the list")
with col_b: line    = st.number_input("Points Line", min_value=0.0, value=24.5, step=0.5)
with col_c: side    = st.selectbox("Over / Under", ["Over", "Under"])
with col_d: n_games = st.selectbox("Sample", [5, 10, 15], index=1)
with col_e: season  = st.text_input("Season", value="2025-26")
...
if fetch:
    ...
    try:
        with st.spinner("Fetching game logs..."):
            st.session_state.logs = fetch_with_retries(lambda: get_last_n_games(player_id=player_id, season=season, n=n_games))
    ...
    if st.session_state.logs is None and manual_mode:
        ...
        st.session_state.logs = pd.DataFrame({"GAME_DATE": [None]*10, "MATCHUP": [None]*10, "MIN": [None]*10,
...
if st.session_state.logs is not None:
    ...
    opp_abbr, game_date, is_b2b = get_next_opponent(player_team)
    ...
    try:
        ...
        for day_offset in [0, 1, 2]:
            ...
            g = scoreboardv2.ScoreboardV2(game_date=check_date, league_id="00", day_offset=0, timeout=10).get_data_frames()[0]
            if not g.empty:
    ...
    # Blend venue into rate
    venue_blended, venue_applied = blend_venue_rate(weighted_base, splits, tonight_venue)

    # ── Stat cards ────────────────────────────────────────────────────────────
    st.markdown(f"<div class='section-header'>{full_name}</div>", unsafe_allow_html=True)
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.markdown(f"<div class='stat-card'><div class='stat-label'>Avg PTS (L{n_games})</div><div class='stat-value orange'>{sample_avg_pts:.1f}</div></div>", unsafe_allow_html=True)
    with m2:
        c = "green" if weighted_base >= 0.6 else ("yellow" if weighted_base >= 0.5 else "red")
        st.markdown(f"<div class='stat-card'><div class='stat-label'>Weighted Hit Rate</div><div class='stat-value {c}'>{weighted_base:.0%}</div></div>", unsafe_allow_html=True)
    with m3:
        c = "green" if consistency >= 0.5 else ("yellow" if consistency >= 0.35 else "red")
        st.markdown(f"<div class='stat-card'><div class='stat-label'>Consistency Score</div><div class='stat-value {c}'>{consistency:.0%}</div></div>", unsafe_allow_html=True)
    with m4:
        c = "green" if venue_blended >= 0.6 else ("yellow" if venue_blended >= 0.5 else "red")
        icon = "🏠" if tonight_venue == "Home" else ("✈️" if tonight_venue == "Away" else "")
        lbl  = f"{'Home' if tonight_venue == 'Home' else 'Away'}-Adj Rate" if venue_applied else "Venue Rate"
        st.markdown(f"<div class='stat-card'><div class='stat-label'>{lbl} {icon}</div><div class='stat-value {c}'>{venue_blended:.0%}</div></div>", unsafe_allow_html=True)
    ...
    if opp_abbr and opp_pts:
        ...
        sched_label = f"Next game vs {opp_abbr}{date_str}" if game_date else f"Most recent opp: {opp_abbr}"
        st.markdown(f"""
        <div class='defense-card'>
            <div>
                <div class='stat-label'>{sched_label}</div>
                <div style='font-size:1.1rem; font-weight:700; color:#f1f5f9; margin-top:4px;'>
                    {opp_pts:.1f} pts allowed/game
                    <span style='font-family:DM Mono; font-size:0.72rem; color:#475569; margin-left:8px;'>league avg {league_avg}</span>
                </div>
            </div>
    ...
    if is_b2b:
        st.markdown("""
        <div style='background:#1c1005; border:1px solid #9a3412; border-radius:10px;
                    padding:0.6rem 1.1rem; margin:0.5rem 0; display:flex; align-items:center; gap:10px;'>
            <span style='font-size:1rem;'>⚠️</span>
            <div>
                <span style='font-family:DM Mono; font-size:0.7rem; color:#f97316; font-weight:600; letter-spacing:0.05em;'>BACK-TO-BACK</span>
                <span style='font-family:DM Mono; font-size:0.7rem; color:#94a3b8; margin-left:8px;'>Player is on second night of a B2B — scoring averages typically drop 2-4 pts</span>
            </div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown(f"<div class='flag-row'>{flag_pill('MIN', min_flag)}{flag_pill('FGA', fga_flag)}{flag_pill('PTS', pts_flag)}</div>", unsafe_allow_html=True)
    st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
    ...
    if splits["home_games"] > 0 or splits["away_games"] > 0:
        ...
        if tonight_venue:
            ...
            venue_note = f"<span style='background:{vc}22; color:{vc}; font-family:DM Mono; font-size:0.7rem; padding:3px 10px; border-radius:999px; border:1px solid {vc}44; margin-left:8px;'>Tonight: {tonight_venue}</span>"
        ha1, ha2 = st.columns(2)
        with ha1:
            if splits["home_games"] >= 2:
                hp = splits["home_rate"]
                hc = "#22c55e" if hp >= 0.6 else ("#eab308" if hp >= 0.5 else "#ef4444")
                used = " · used in verdict" if venue_applied and tonight_venue == "Home" else ""
                st.markdown(f"""<div class='stat-card' style='border-color:{"#166534" if tonight_venue=="Home" else "#1e293b"};'>
                    <div class='stat-label'>Home {venue_note if tonight_venue=="Home" else ""}</div>
                    <div style='display:flex;align-items:baseline;gap:12px;margin-top:4px;'>
                        <div class='stat-value' style='color:{hc};'>{hp:.0%}</div>
                        <div style='font-family:DM Mono;font-size:0.72rem;color:#475569;'>hit rate</div>
                    </div>
                    <div style='font-family:DM Mono;font-size:0.72rem;color:#475569;margin-top:4px;'>{splits["home_avg"]} avg pts · {splits["home_games"]} games{used}</div>
                </div>""", unsafe_allow_html=True)
            else:
        ...
        with ha2:
            if splits["away_games"] >= 2:
                ap = splits["away_rate"]
                ac = "#22c55e" if ap >= 0.6 else ("#eab308" if ap >= 0.5 else "#ef4444")
                used = " · used in verdict" if venue_applied and tonight_venue == "Away" else ""
                st.markdown(f"""<div class='stat-card' style='border-color:{"#166534" if tonight_venue=="Away" else "#1e293b"};'>
                    <div class='stat-label'>Away {venue_note if tonight_venue=="Away" else ""}</div>
                    <div style='display:flex;align-items:baseline;gap:12px;margin-top:4px;'>
                        <div class='stat-value' style='color:{ac};'>{ap:.0%}</div>
                        <div style='font-family:DM Mono;font-size:0.72rem;color:#475569;'>hit rate</div>
                    </div>
                    <div style='font-family:DM Mono;font-size:0.72rem;color:#475569;margin-top:4px;'>{splits["away_avg"]} avg pts · {splits["away_games"]} games{used}</div>
                </div>""", unsafe_allow_html=True)
            else:
    ...
    st.plotly_chart(build_points_chart(logs, full_name, line, sample_avg_pts), use_container_width=True)
    ...
    if opp_abbr:
        dd = game_date if game_date else ""
        bc = matchup_auto.lower()
        bt = {"Good": "Weak defense", "Bad": "Strong defense", "Neutral": "Average defense"}[matchup_auto]
        ops = f"{opp_pts:.1f}" if opp_pts else "N/A"
        st.markdown(
            "<div style='background:#0f172a;border:1px solid #1e293b;border-radius:10px;"
            "padding:0.75rem 1.2rem;margin-bottom:1rem;display:flex;align-items:center;"
            "justify-content:space-between;flex-wrap:wrap;gap:0.5rem;'>"
            "<div><div style='font-family:DM Mono;font-size:0.65rem;color:#475569;"
            ...
            "<span style='font-family:DM Mono;font-size:0.75rem;color:#475569;font-weight:400;margin-left:8px;'>" + dd + "</span></div></div>"
            "<span class='defense-badge " + bc + "'>" + bt + " · " + ops + " pts/g allowed</span></div>",
            unsafe_allow_html=True)
    ...
    with pc1:
        ...
        matchup_sel = st.selectbox("Matchup 🤖", matchup_options, index=matchup_options.index(matchup_auto),
    ...
    with st.expander("⚙️  Advanced overrides"):
        ...
        with ac1: minutes_sel = st.selectbox("Minutes", ["Okay","Strong","Risk"], index=["Okay","Strong","Risk"].index(minutes_suggest))
        with ac2: role_sel    = st.selectbox("Role",    ["Okay","Strong","Risk"], index=["Okay","Strong","Risk"].index(role_suggest))
        with ac3: shots_sel   = st.selectbox("Shots",   ["Medium","High","Low"],  index=["Medium","High","Low"].index(shots_suggest))

    context = {"minutes": minutes_sel, "role": role_sel, "shots": shots_sel, "matchup": matchup_sel, "script": script_sel}

    # Apply context on top of venue-blended rate
    adjusted = apply_adjustments(venue_blended, context)

    # Back-to-back penalty — compress margin toward 50% by 8%
    # Scoring drops ~2-4 pts on B2B which meaningfully affects hit rate
    if is_b2b:
        margin   = adjusted - 0.5
    ...
    <div class='verdict-banner {css}'>
        ...
        <div style='display:flex;gap:2rem;flex-wrap:wrap;'>
            <div><div class='verdict-label'>Adjusted Hit Rate</div><div style='font-size:1.4rem;font-weight:800;color:#f1f5f9;'>{adjusted:.0%}</div></div>
            <div><div class='verdict-label'>Edge vs Line</div><div style='font-size:1.4rem;font-weight:800;color:{"#22c55e" if line_diff>0 else "#ef4444"};'>{line_diff:+.1f}</div></div>
            <div><div class='verdict-label'>Consistency</div><div style='font-size:1.4rem;font-weight:800;color:#f1f5f9;'>{consistency:.0%}</div></div>
        </div>
    </div>""", unsafe_allow_html=True)

    with st.expander("🔬  How this verdict was calculated"):
        venue_str = f"Tonight: {tonight_venue} · Venue split {'applied' if venue_applied else 'not applied (< 4 games)'}"
        b2b_str   = "⚠️ Back-to-back penalty applied (-8% margin)" if is_b2b else "No B2B"
        st.markdown(f"""<div class='model-note'>
        Raw hit rate: {baseline:.0%} &nbsp;·&nbsp; Weighted (recency): {weighted_base:.0%} &nbsp;·&nbsp;
        Venue-blended: {venue_blended:.0%} &nbsp;·&nbsp; Final after context: {adjusted:.0%}
        <br>{venue_str} &nbsp;·&nbsp; {b2b_str}
        </div>""", unsafe_allow_html=True)
                ...
                with st.spinner("Analyzing..."):
                    try:
                        prompt = build_analysis_prompt(
                            ...
                            venue_blended=venue_blended, adjusted=adjusted, tier=tier,
                            avg_pts=sample_avg_pts, avg_min=avg_min, avg_fga=avg_fga, consistency=consistency,
    ...
    with ex1:
        out = logs.copy()
        for i, (k, v) in enumerate([("PLAYER", full_name), ("LINE", line), ("SIDE", side),
            ("OPPONENT", opp_abbr or ""), ("TONIGHT_VENUE", tonight_venue or ""),
            ("OPP_PTS_ALLOWED", opp_pts or ""), ("MATCHUP_QUALITY", matchup_sel),
            ("RAW_HIT_RATE", baseline), ("WEIGHTED_HIT_RATE", weighted_base),
            ("VENUE_BLENDED", venue_blended), ("ADJUSTED_RATE", adjusted),
            ("CONSISTENCY", consistency), ("TIER", tier)]):
            out.insert(i, k, v)
        st.download_button("⬇  Download CSV", data=out.to_csv(index=False).encode("utf-8"),
                           file_name="prop_report.csv", mime="text/csv")
    with ex2:
        if st.button("➕  Add to Prop Tracker"):
            entry = {"Player": full_name, "Line": f"{line} {side}", "Opponent": opp_abbr or "—",
                     "Venue": tonight_venue or "—", "Matchup": matchup_sel,
                     "Avg PTS": round(sample_avg_pts, 1), "Hit Rate": f"{weighted_base:.0%}",
                     "Adjusted": f"{adjusted:.0%}", "Consistency": f"{consistency:.0%}", "Verdict": tier}
...
if not st.session_state.tracker:
    st.markdown("""<div style='background:#0f172a;border:1px dashed #1e293b;border-radius:12px;padding:1.5rem;text-align:center;'>
        <div style='font-family:DM Mono;font-size:0.75rem;color:#334155;'>No props tracked yet</div>
        <div style='font-size:0.85rem;color:#475569;margin-top:4px;'>Analyze a player then click ➕ Add to Prop Tracker</div>
    </div>""", unsafe_allow_html=True)
else:
    tier_css   = {"Strong Over":"green","Lean Over":"yellow","Lean Under":"orange","Strong Under":"red","Pass":"gray"}
    tier_emoji = {"Strong Over":"🟢","Lean Over":"🟡","Lean Under":"🟠","Strong Under":"🔴","Pass":"⚪"}
    ...
    for i, entry in enumerate(st.session_state.tracker):
        ...
        venue_tag = f" · {entry.get('Venue','—')}" if entry.get("Venue") and entry.get("Venue") != "—" else ""
        col_card, col_remove = st.columns([11, 1])
        with col_card:
            st.markdown(f"""
            <div class='verdict-banner {css}' style='margin:0.3rem 0;padding:1rem 1.4rem;'>
                <div>
                    <div class='verdict-label'>{entry["Line"]} · vs {entry["Opponent"]}{venue_tag}</div>
                    <div style='font-size:1.1rem;font-weight:800;color:#f1f5f9;letter-spacing:-0.5px;'>{entry["Player"]}</div>
                </div>
                <div style='display:flex;gap:1.5rem;flex-wrap:wrap;align-items:center;'>
                    <div><div class='verdict-label'>Verdict</div><div class='verdict-tier {css}' style='font-size:1rem;'>{emoji} {t}</div></div>
    ...
    with tc1:
        tracker_csv = pd.DataFrame(st.session_state.tracker).to_csv(index=False).encode("utf-8")
