...
:root {
    ...
    --bg3: #151d28;
    --bg4: #1b2533;
    --border: rgba(203,213,225,0.075);
    --border2: rgba(203,213,225,0.14);
    --accent: #20c7d2;
    --accent2: #128d98;
    --accent3: #76e4ec;
    --accent-glow: rgba(32,199,210,0.16);
    --text: #edf3f8;
    --text2: #a7b4c4;
    --text3: #6f7f91;
...
[data-testid="stAppViewContainer"] {
    background:
        radial-gradient(circle at 50% -10%, rgba(32,199,210,0.09), transparent 34%),
        linear-gradient(180deg, #080b10 0%, #0b1017 52%, #080b10 100%) !important;
...
.pl-header {
    ...
    border-bottom: 1px solid rgba(203,213,225,0.10) !important;
    box-shadow: 0 10px 34px rgba(0,0,0,0.26) !important;
}
.pl-header::after {
    height: 1px !important;
    background: linear-gradient(90deg, transparent, rgba(118,228,236,0.62), transparent) !important;
    animation: none !important;
}
.pl-logo {
...
.pl-badge {
    background: rgba(32,199,210,0.08) !important;
    border-color: rgba(32,199,210,0.22) !important;
    box-shadow: none !important;
}
.pl-badge::after,
.v55-hero::before,
...
.section-header {
    ...
    border-bottom-color: rgba(203,213,225,0.09) !important;
...
.v55-hero {
    background: linear-gradient(135deg, rgba(16,22,31,0.96), rgba(14,20,28,0.90)) !important;
    border: 1px solid rgba(203,213,225,0.10) !important;
    border-radius: 10px !important;
    box-shadow: var(--shadow-md) !important;
...
div[data-testid="stExpander"],
.edge-card-v55 {
    background: rgba(16,22,31,0.86) !important;
    border-color: rgba(203,213,225,0.09) !important;
    border-radius: 8px !important;
    box-shadow: var(--shadow-sm) !important;
    animation: none !important;
}
...
.verdict-banner:hover {
    ...
    border-color: rgba(118,228,236,0.22) !important;
    box-shadow: var(--shadow-md) !important;
}
.stat-value,
.edge-score-item-v55 strong,
...
.edge-card-v55.strong {
    border-left-color: rgba(0,232,150,0.95) !important;
    background: rgba(15,24,22,0.90) !important;
}
.edge-card-v55.lean {
    border-left-color: rgba(255,193,7,0.92) !important;
    background: rgba(25,23,17,0.90) !important;
}
.edge-player-v55 {
    font-size: 1rem !important;
...
.parlay-leg-pick {
    ...
    background: rgba(203,213,225,0.055) !important;
    border-color: rgba(203,213,225,0.08) !important;
}
.edge-pill-v55.accent,
.flag-pill.up {
    background: rgba(32,199,210,0.075) !important;
    border-color: rgba(32,199,210,0.18) !important;
}
.edge-pill-v55.warn,
.flag-pill.down {
    background: rgba(255,112,67,0.075) !important;
    border-color: rgba(255,112,67,0.18) !important;
}
.edge-score-item-v55 {
    background: rgba(8,11,16,0.34) !important;
    border: 1px solid rgba(203,213,225,0.07) !important;
    border-radius: 8px !important;
...
.verdict-banner.green { background: linear-gradient(135deg, rgba(4,32,21,0.96), rgba(16,22,31,0.92)) !important; }
.verdict-banner.yellow { background: linear-gradient(135deg, rgba(35,27,5,0.96), rgba(16,22,31,0.92)) !important; }
.verdict-banner.orange { background: linear-gradient(135deg, rgba(38,19,6,0.96), rgba(16,22,31,0.92)) !important; }
.verdict-banner.red { background: linear-gradient(135deg, rgba(38,8,14,0.96), rgba(16,22,31,0.92)) !important; }
.stButton > button {
    border-radius: 8px !important;
    box-shadow: none !important;
    letter-spacing: 0.04em !important;
    background: linear-gradient(135deg, #2ad2dc 0%, #1595a0 100%) !important;
}
.stButton > button:hover {
    transform: translateY(-1px) !important;
    box-shadow: 0 12px 24px rgba(32,199,210,0.18) !important;
}
button[data-testid="baseButton-secondary"] {
    background: rgba(16,22,31,0.86) !important;
    border-color: rgba(203,213,225,0.10) !important;
    box-shadow: none !important;
}
div[data-testid="stSelectbox"] > div > div,
div[data-testid="stNumberInput"] input,
div[data-testid="stTextInput"] input {
    background: rgba(12,17,24,0.92) !important;
...
def add_to_pick_list_and_tracker(leg: dict, entry: dict) -> None:
    """Add a shortlist pick and preserve its probability for calibration."""
    add_to_pick_list(leg)
    try:
        ...
        existing_idx = next(
            (
                i for i, old in enumerate(tracker)
        ...
        if existing_idx is not None:
            old_id = tracker[existing_idx].get("id")
            tracker[existing_idx] = {**entry, **({"id": old_id} if old_id else {})}
            if old_id:
                delete_from_supabase(old_id)
            new_id = save_to_supabase(st.session_state.session_id, entry)
            if new_id:
                tracker[existing_idx]["id"] = new_id
        else:
            new_id = save_to_supabase(st.session_state.session_id, entry)
            if new_id:
                entry = {**entry, "id": new_id}
            tracker.append(entry)
        st.session_state.parlay_notice = (
            f"Added {player} to your pick list and calibration tracker."
        )
...
def is_mlb_hitter_fantasy_prop(stat: str) -> bool:
    return normalize_mlb_pitcher_prop_stat(stat) == "Hitter Fantasy Score"


def normalize_wnba_prop_stat(stat: str) -> str:
    """Map PrizePicks WNBA market labels to the analyzer's canonical names."""
    s = re.sub(r"[^a-z0-9]+", " ", str(stat or "").lower()).strip()
    compact = s.replace(" ", "")
    aliases = {
        "points": "Points", "pts": "Points",
        "rebounds": "Rebounds", "rebound": "Rebounds", "rebs": "Rebounds", "reb": "Rebounds",
        "assists": "Assists", "assist": "Assists", "asts": "Assists", "ast": "Assists",
        "3ptmade": "3-Pointers Made", "3ptsmade": "3-Pointers Made", "3pm": "3-Pointers Made",
        "3pointersmade": "3-Pointers Made", "threepointersmade": "3-Pointers Made",
        "ptsrebsasts": "Pts + Reb + Ast", "pointsreboundsassists": "Pts + Reb + Ast", "pra": "Pts + Reb + Ast",
        "ptsrebs": "Points + Rebounds", "pointsrebounds": "Points + Rebounds", "pr": "Points + Rebounds",
        "ptsasts": "Points + Assists", "pointsassists": "Points + Assists", "pa": "Points + Assists",
        "rebsasts": "Rebounds + Assists", "reboundsassists": "Rebounds + Assists", "ra": "Rebounds + Assists",
    }
    return aliases.get(compact, str(stat or ""))


def is_wnba_supported_prop(stat: str) -> bool:
    return normalize_wnba_prop_stat(stat) in WNBA_STAT_COLUMNS


def is_wnba_points_prop(stat: str) -> bool:
    return normalize_wnba_prop_stat(stat) == "Points"


def is_sane_wnba_prop_line(stat: str, line) -> bool:
    """Reject cross-mapped or malformed WNBA lines before model execution."""
    try:
        ...
        return False
    ranges = {
        "Points": (0.5, 40.5), "Rebounds": (0.5, 20.5),
        "Assists": (0.5, 15.5), "3-Pointers Made": (0.5, 8.5),
        "Pts + Reb + Ast": (4.5, 65.5), "Points + Rebounds": (2.5, 55.5),
        "Points + Assists": (2.5, 52.5), "Rebounds + Assists": (1.5, 32.5),
    }
    low, high = ranges.get(market, (1.0, 0.0))
...
def is_sane_mlb_pitcher_prop_line(stat: str, line) -> bool:
    ...
    if stat_label == "Hitter Fantasy Score":
        return 1.5 <= line_val <= 18.5
...
def auto_detect_result(entry: dict) -> Optional[str]:
    ...
    """
    try:
        from datetime import datetime as _auto_datetime
...
if st.session_state.active_sport == "mlb":
    ...
    def mlb_get_pitcher_season_stats(player_name: str) -> dict:
        ...
        try:
            ...
            for season in [current_year, current_year - 1]:
                ...
                try:
                    whole, _, partial = ip_str.partition(".")
                    ip_f = int(whole or 0) + (int(partial or 0) / 3.0)
                except Exception:
                    ip_f = 0.0
                starts = int(stat.get("gamesStarted", 0) or 0)
                if season == current_year and starts < 2 and ip_f < 8:
                    continue

                strikeouts = float(stat.get("strikeOuts", 0) or 0)
                walks = float(stat.get("baseOnBalls", 0) or 0)
                bf = int(stat.get("battersFaced", 0) or 0)
                total_pitches = int(stat.get("numberOfPitches", 0) or 0)
                k9 = round(strikeouts / max(ip_f, 1) * 9, 2)
                bb9 = round(walks / max(ip_f, 1) * 9, 2)
                era = float(stat.get("era", "0") or 0)
                whip = float(stat.get("whip", "0") or 0)
                avg_ip = round(ip_f / max(starts, 1), 1)
                k_rate = round(strikeouts / bf, 3) if bf > 0 else None
                pitches_per_bf = (
                    round(total_pitches / bf, 3)
                    if total_pitches > 0 and bf >= 20 else None
                )
                return {
                    "k9": k9, "swstr": k_rate, "bb9": bb9, "era": era,
                    "avg_ip": avg_ip, "whip": whip, "ip_total": ip_f,
                    "starts": starts, "season": season,
                    "batters_faced": bf, "total_pitches": total_pitches,
    ...
    def mlb_get_opp_plate_profile(opp_abbr: str) -> dict:
        ...
        """
        empty = {"k_rate": None, "bb_rate": None, "pa": None,
                 "pitches_per_pa": None, "profile": "Neutral"}
        if not opp_abbr:
            return empty
        try:
            import requests as _req
            ...
            pitches_seen = int(stat.get("numberOfPitches", 0) or 0)
            pitches_per_pa = (
                pitches_seen / pa if pitches_seen > 0 and pa > 0 else None
            )
            if k_rate >= 0.255 and bb_rate <= 0.085:
                ...
                profile = "Neutral"
            return {
                "k_rate": round(k_rate, 3), "bb_rate": round(bb_rate, 3),
                "pa": denom,
                "pitches_per_pa": round(pitches_per_pa, 3) if pitches_per_pa else None,
                "profile": profile,
            }
        except Exception:
            return empty
    ...
    def mlb_estimate_pitch_count(pitcher_name: str, n: int = 5) -> dict:
        ...
        try:
            ...
            n_starts = len(pitch_counts)
            pitches_per_bf = (
                round(total_valid_pitches / total_valid_bf, 3)
            ...
            if is_outs:
                ...
                if side == "Over":
                    if expected_k is not None:
                        if expected_k <= line + 0.25:
                            add_risk(f"Expected Ks barely clear line ({expected_k:.1f})", 3)
                        elif expected_k <= line + 0.75:
                            add_risk(f"Thin K cushion ({expected_k:.1f})", 2)
                        elif expected_k >= line + 1.5:
                    ...
                    if expected_k is not None:
                        if expected_k >= line - 0.25:
                            add_risk(f"Expected Ks too close for Under ({expected_k:.1f})", 3)
                        elif expected_k >= line - 0.75:
                            add_risk(f"Thin Under cushion ({expected_k:.1f})", 2)
    ...
    def mlb_lineup_bf_projection(
        ...
        try:
            ...
            if pitch_count and pitch_count > 0:
                valid_pitcher_ppbf = (
                    float(pitcher_pitches_per_bf)
                    if pitcher_pitches_per_bf is not None
                    and 3.2 <= float(pitcher_pitches_per_bf) <= 5.0
                    else None
                )
                valid_opp_ppa = (
                    float(opponent_pitches_per_pa)
                    if opponent_pitches_per_pa is not None
                    and 3.2 <= float(opponent_pitches_per_pa) <= 5.0
                    else None
                )
                if valid_pitcher_ppbf and valid_opp_ppa:
                    matchup_ppbf = (0.68 * valid_pitcher_ppbf) + (0.32 * valid_opp_ppa)
                    bf_method = "pitcher P/BF + opponent P/PA"
                elif valid_pitcher_ppbf:
                    matchup_ppbf = valid_pitcher_ppbf
                    bf_method = "pitcher P/BF"
                elif valid_opp_ppa:
                    matchup_ppbf = valid_opp_ppa
                    bf_method = "opponent P/PA"
                else:
                    matchup_ppbf = 3.95
                    bf_method = "league-average P/BF"

                # Walk-heavy opponents tend to extend plate appearances and
                # innings. Keep this adjustment small because P/PA already
                # captures much of the same behavior when available.
                if opponent_bb_rate is not None:
                    bb_rate = float(opponent_bb_rate)
                    if bb_rate >= 0.105:
                        matchup_ppbf += 0.08
                    elif bb_rate <= 0.070:
                        matchup_ppbf -= 0.05
                matchup_ppbf = max(3.35, min(4.55, matchup_ppbf))
                expected_bf = float(pitch_count) / matchup_ppbf
            elif avg_ip and avg_ip > 0:
                expected_bf = float(avg_ip) * 4.25
                matchup_ppbf = None
                bf_method = "innings fallback"
            else:
                expected_bf = None
                matchup_ppbf = None
                bf_method = ""
    ...
    @st.cache_data(ttl=86400, show_spinner=False)
    def _mlb_fetch_hitter_list() -> list:
        """Fetch active MLB hitters for Hitter Fantasy Score analysis."""
        import requests as _req, datetime as _dtx
        season = _dtx.datetime.now().year
        hitters = set()
        try:
            for _gt in ["R", "S"]:
                ...
                if r.ok:
                    for p in r.json().get("people", []):
                        pos = p.get("primaryPosition", {}).get("code", "")
                        name = p.get("fullName", "").strip()
                        if name and len(name) > 3 and pos not in ("1", "P"):
                            hitters.add(name)
                    if hitters:
                        break
        except Exception:
            pass
        _seed = [
            "Shohei Ohtani", "Aaron Judge", "Juan Soto", "Mookie Betts",
            "Freddie Freeman", "Bobby Witt Jr.", "Gunnar Henderson",
            "Yordan Alvarez", "Kyle Tucker", "Elly De La Cruz",
            "Jose Ramirez", "Vladimir Guerrero Jr.", "Corbin Carroll",
            "Julio Rodriguez", "Fernando Tatis Jr.", "Rafael Devers",
            "Francisco Lindor", "Pete Alonso", "Bryce Harper",
            "Trea Turner", "Corey Seager", "Ketel Marte", "Oneil Cruz",
            "Jazz Chisholm Jr.", "Jackson Chourio", "Adley Rutschman",
        ]
        for n in _seed:
            hitters.add(n)
    ...
    def mlb_find_player_basic(player_name: str, hitter_only: bool = False) -> dict:
        ...
        try:
            import requests as _req, datetime as _dtx
            season = _dtx.datetime.now().year
            _norm = lambda s: str(s or "").lower().strip().replace(".", "").replace("-", " ")
            ...
            if tr.ok:
                team_map = {
                    int(t.get("id")): mlb_norm_abbr(t.get("abbreviation", ""))
                    for t in tr.json().get("teams", [])
                    if t.get("id")
                }
            ...
            people = r.json().get("people", [])
            matches = (
                [p for p in people if _norm(p.get("fullName", "")) == target] or
                [p for p in people if all(pt in _norm(p.get("fullName", "")) for pt in parts)] or
                [p for p in people if last in _norm(p.get("fullName", "")) and len(last) >= 5]
            )
            for p in matches:
                pos = p.get("primaryPosition", {}).get("code", "")
                if hitter_only and pos in ("1", "P"):
                    continue
    ...
    def mlb_hitter_fantasy_points_from_stat(st: dict) -> float:
        """PrizePicks MLB hitter fantasy scoring."""
        try:
            ...
            return (
                ...
                t * 8 +
                hr * 10 +
                int(st.get("runs", 0) or 0) * 2 +
                int(st.get("rbi", 0) or 0) * 2 +
                int(st.get("baseOnBalls", 0) or 0) * 2 +
                int(st.get("hitByPitch", 0) or 0) * 2 +
                int(st.get("stolenBases", 0) or 0) * 5
            )
        except Exception:
            return 0.0
    ...
    @st.cache_data(ttl=1800, show_spinner=False)
    def mlb_get_hitter_logs(player_name: str, n: int = 12) -> pd.DataFrame:
        """Fetch recent hitter game logs and compute PrizePicks hitter fantasy score."""
        cols = ["DATE", "OPP", "PA", "H", "TB", "R", "RBI", "BB", "HBP", "SB", "HR", "FS"]
        ...
        try:
            ...
            for yr in [season, season - 1]:
                r = _req.get(
                    f"https://statsapi.mlb.com/api/v1/people/{basic['id']}/stats",
                    params={"stats": "gameLog", "group": "hitting", "season": yr, "gameType": "R"},
                    timeout=10,
                )
                if not r.ok:
                    continue
                for s in r.json().get("stats", [{}])[0].get("splits", []):
                    ...
                    date = s.get("date", "")
                    try:
                        dt = _dtx.date.fromisoformat(date[:10])
                    except Exception:
                        continue
                    ...
                    all_rows.append({
                        ...
                        "HBP": int(stt.get("hitByPitch", 0) or 0),
                        "SB": int(stt.get("stolenBases", 0) or 0),
                        "HR": int(stt.get("homeRuns", 0) or 0),
                        "FS": round(fs, 1),
                    })
                if len(all_rows) >= n:
                    break
            ...
            return df.sort_values("DATE", ascending=False).head(n).reset_index(drop=True)[cols]
        except Exception:
            return empty
    ...
    def mlb_get_team_next_game(team_abbr: str) -> dict:
        ...
        if not team_abbr:
            return empty
        try:
            import requests as _req, datetime as _dtx, pytz
            et = pytz.timezone("America/New_York")
            target = mlb_norm_abbr(team_abbr)
            for offset in range(0, 2):
                date_str = (_dtx.datetime.now(et) + _dtx.timedelta(days=offset)).strftime("%Y-%m-%d")
                r = _req.get(
                    "https://statsapi.mlb.com/api/v1/schedule",
                ...
                for day in r.json().get("dates", []):
                    for game in day.get("games", []):
                        state = str(game.get("status", {}).get("abstractGameState", "")).lower()
                        if state in ("final", "live"):
                            continue
                        teams = game.get("teams", {})
                        home = mlb_norm_abbr(teams.get("home", {}).get("team", {}).get("abbreviation", ""))
                        away = mlb_norm_abbr(teams.get("away", {}).get("team", {}).get("abbreviation", ""))
                        if target not in (home, away):
                            continue
                        side = "home" if target == home else "away"
                        opp_side = "away" if side == "home" else "home"
                        opp = away if side == "home" else home
                        prob = teams.get(opp_side, {}).get("probablePitcher", {})
                        phand = "R"
                        if prob.get("id"):
                            try:
                                pr = _req.get(f"https://statsapi.mlb.com/api/v1/people/{prob.get('id')}", timeout=5)
                                if pr.ok:
                                    phand = pr.json().get("people", [{}])[0].get("pitchHand", {}).get("code", "R")
                            except Exception:
                                pass
                        return {
                            "opp": opp,
    ...
    def mlb_render_hitter_fantasy_analysis(player_name: str, line: float, side: str):
        ...
        vals = pd.to_numeric(logs["FS"], errors="coerce").dropna().reset_index(drop=True)
        avg_fs = float(vals.mean())
        edge = avg_fs - float(line)
        whr = mlb_weighted_hr(logs, line, "FS", side)
        cons = max(0.10, min(0.95, 1.0 - ((float(vals.std()) / avg_fs) * 0.55 if avg_fs > 0 and len(vals) > 1 else 0.35)))
        base = mlb_calibrated_hit_base(whr, len(vals), cons)
        l3 = float(vals.head(3).mean()) if len(vals) >= 3 else avg_fs
        l3_diff = l3 - avg_fs
        ...
        if side == "Under":
            adj = 1.0 - adj
        adj = max(0.05, min(0.92, adj))
        if cons < 0.42 and abs(edge) < 2.0:
            adj = min(adj, 0.66)

        tier = (
            "Strong Over" if side == "Over" and adj >= 0.72 else
            "Lean Over" if side == "Over" and adj >= 0.55 else
            "Strong Under" if side == "Under" and adj >= 0.72 else
        ...
        with c1:
            st.markdown(f"<div class='stat-card'><div class='stat-label'>Avg Fantasy Score</div><div class='stat-value {'green' if edge>0 else 'red'}'>{avg_fs:.1f}</div><div class='stat-hint'>Line {line} · edge {edge:+.1f}</div></div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div class='stat-card'><div class='stat-label'>Weighted Hit Rate</div><div class='stat-value {'green' if whr>=0.6 else 'yellow' if whr>=0.5 else 'red'}'>{whr:.0%}</div><div class='stat-hint'>L{len(vals)} hitter logs</div></div>", unsafe_allow_html=True)
        with c3:
    ...
    if mlb_pitcher and mlb_prop == "Hitter Fantasy Score":
        ...
        _tonight = mlb_get_team_next_game(_mlb_basic.get("team", ""))
    else:
        _mlb_basic = {}
    ...
    if mlb_pitcher and _tonight:
        ...
        _role_note = "may not be scheduled to pitch" if mlb_prop == "Strikeouts" else "does not have a detected game today"
        st.markdown(
            f"<div style='background:#111;border:1px solid #1e2a3a;border-left:3px solid #f97316;"
            f"padding:0.5rem 1rem;margin-bottom:0.5rem;font-family:JetBrains Mono,monospace;"
            f"font-size:0.68rem;color:#f97316;'>⚠️ No game found for today — {mlb_pitcher} {_role_note}{_last_str}."
    ...
    if mlb_fetch and mlb_pitcher and mlb_prop == "Strikeouts":
        ...
        if mlb_logs.empty:
            ...
            # 1. Current season stats API once it has 2 starts or 8 IP
            ...
            _needs_fallback = (
                _k9 == 0.0 or
                (_pstats.get("starts", 0) < 2 and _k9_ip < 8)
            )
                        ...
                        if _pp:
                            for _prev_yr in [_dtx2.datetime.now().year - 1,
                                ...
                                if _prev.ok:
                                    ...
                                    try:
                                        _whole, _, _partial = _ip_s.partition(".")
                                        _ip_f = int(_whole or 0) + (int(_partial or 0) / 3.0)
                                    except Exception:
                                        _ip_f = 0.0
                ...
                if _k9 == 0.0 and not mlb_logs.empty:
                    try:
                        ...
                        _log_ips = mlb_logs["IP"].apply(lambda x: (
                            int(str(x).partition(".")[0] or 0) +
                            (int(str(x).partition(".")[2] or 0) / 3.0)
            ...
            _quality_expected_k = _combined_proj_expected_k or _bf_expected_k
            if _quality_expected_k is not None:
                _bf_cushion_for_quality = (
                    _quality_expected_k - mlb_line
                    if mlb_side == "Over" else
                    mlb_line - _quality_expected_k
                )
            ...
            with st.expander("🔬  Show signal breakdown (debug)"):
                ...
                _mlb_signals = [
                    ...
                    ("BF efficiency",              (
                                                  ...
                                                  None,
                                                  (
                                                      f"{_bf_proj.get('bf_method','')} · "
                                                      f"expected {_bf_proj.get('expected_bf'):.1f} BF"
                                                      if _bf_proj.get("expected_bf") is not None
                                                      else "Expected batters faced unavailable"
                                                  )),
...
if st.session_state.active_sport == "edge":
    ...
    def scanner_grade_and_reasons(result: dict) -> Tuple[str, str, List[str]]:
        ...
        if result.get("sport") == "MLB":
            ...
            is_hitter_fs = is_mlb_hitter_fantasy_prop(result.get("stat", ""))
            if result.get("l3") is not None:
                reasons.append(f"L3 avg {result['l3']}")
            if result.get("opp") == "TBD":
                reasons.append("Opponent pending")
            if result.get("pa_avg") is not None and is_hitter_fs:
                reasons.append(f"PA path {result['pa_avg']}")
            if result.get("expected_fs") is not None and is_hitter_fs:
                reasons.append(f"Expected FS {result['expected_fs']}")
            if result.get("opp_k") is not None and not is_outs_prop and not is_hitter_fs:
            ...
            if result.get("defense_label") and result.get("defense_label") != "Neutral":
                reasons.append(f"{result['defense_label']} defense")
            if result.get("projection") is not None:
                reasons.append(f"Projection {result['projection']}")
            if result.get("injury_status") not in (None, "", "Active", "Active/Unlisted"):
    ...
    @st.cache_data(ttl=7200, show_spinner=False)
    def _scanner_find_mlb_hitter(player_name: str) -> dict:
        """Find active MLB hitter id/team for Edge Scanner Hitter Fantasy Score."""
        empty = {"id": None, "name": player_name, "team": ""}
        try:
            import requests as _req, datetime as _dt
            target = re.sub(r"[^a-z0-9 ]", "", str(player_name).lower()).strip()
            parts = [p for p in target.split() if p]
            if not parts:
                return empty
            r = _req.get(
                "https://statsapi.mlb.com/api/v1/sports/1/players",
                params={"season": _dt.datetime.now().year, "gameType": "R"},
                timeout=8,
            )
            if not r.ok:
                return empty
            best = None
            for p in r.json().get("people", []):
                pos_type = str(p.get("primaryPosition", {}).get("type", "")).lower()
                if "pitcher" in pos_type:
                    continue
                nm = str(p.get("fullName", ""))
                norm = re.sub(r"[^a-z0-9 ]", "", nm.lower()).strip()
                if norm == target or all(part in norm for part in parts):
                    best = p
                    break
            if not best:
                return empty
            team = best.get("currentTeam", {}).get("abbreviation", "")
            return {
                "id": best.get("id"),
                "name": best.get("fullName", player_name),
                "team": _scanner_norm_mlb_team(team),
            }
        except Exception as e:
            record_debug_error("mlb.edge.hitter.find", e)
            return empty


    def _scanner_hitter_fs_from_stat(st_row: dict) -> float:
        """PrizePicks-style hitter fantasy score from MLB game-log hitting stats."""
        try:
            hits = float(st_row.get("hits", 0) or 0)
            doubles = float(st_row.get("doubles", 0) or 0)
            triples = float(st_row.get("triples", 0) or 0)
            homers = float(st_row.get("homeRuns", 0) or 0)
            singles = max(0.0, hits - doubles - triples - homers)
            runs = float(st_row.get("runs", 0) or 0)
            rbi = float(st_row.get("rbi", st_row.get("runsBattedIn", 0)) or 0)
            walks = float(st_row.get("baseOnBalls", 0) or 0)
            hbp = float(st_row.get("hitByPitch", 0) or 0)
            steals = float(st_row.get("stolenBases", 0) or 0)
            return (
                singles * 3 + doubles * 5 + triples * 8 + homers * 10 +
                runs * 2 + rbi * 2 + walks * 2 + hbp * 2 + steals * 5
            )
        except Exception:
            return 0.0
    ...
    @st.cache_data(ttl=1800, show_spinner=False)
    def _scanner_get_hitter_logs(player_name: str, n: int = 14) -> pd.DataFrame:
        """Recent hitter logs with fantasy score and PA for Edge Scanner."""
        cols = ["DATE", "FS", "PA", "AB", "H", "BB", "R", "RBI", "HR", "SB"]
        empty = pd.DataFrame(columns=cols)
        try:
            import requests as _req, datetime as _dt
            basic = _scanner_find_mlb_hitter(player_name)
            pid = basic.get("id")
            if not pid:
                return empty
            ...
            for yr in [season, season - 1]:
                r = _req.get(
                    f"https://statsapi.mlb.com/api/v1/people/{pid}/stats",
                    params={"stats": "gameLog", "group": "hitting", "season": yr},
                    timeout=10,
                )
                if not r.ok:
                    continue
                ...
                for sp in splits:
                    st_row = sp.get("stat", {}) or {}
                    date = sp.get("date", "")
                    pa = float(st_row.get("plateAppearances", 0) or 0)
                    rows.append({
                        "DATE": date,
                        "FS": _scanner_hitter_fs_from_stat(st_row),
                        "PA": pa,
                        "AB": float(st_row.get("atBats", 0) or 0),
                        "H": float(st_row.get("hits", 0) or 0),
                        "BB": float(st_row.get("baseOnBalls", 0) or 0),
                        "R": float(st_row.get("runs", 0) or 0),
                        "RBI": float(st_row.get("rbi", st_row.get("runsBattedIn", 0)) or 0),
                        "HR": float(st_row.get("homeRuns", 0) or 0),
                        "SB": float(st_row.get("stolenBases", 0) or 0),
                    })
                if len(rows) >= n:
                    break
            if not rows:
            ...
            df = pd.DataFrame(rows)
            df["DATE"] = pd.to_datetime(df["DATE"], errors="coerce")
            df = df.dropna(subset=["DATE"]).sort_values("DATE", ascending=False).head(n).reset_index(drop=True)
            return df[cols]
        except Exception as e:
            record_debug_error("mlb.edge.hitter.logs", e)
            return empty
    ...
    def _scanner_get_hitter_game_context(team_abbr: str) -> dict:
        ...
        if not team_abbr:
            return empty
        try:
            import requests as _req, datetime as _dt, pytz as _pytz
            ...
            for offset in range(0, 3):
                ...
                for day in r.json().get("dates", []):
                    for game in day.get("games", []):
                        teams = game.get("teams", {})
                        h = teams.get("home", {}).get("team", {})
                        a = teams.get("away", {}).get("team", {})
                        home = _scanner_norm_mlb_team(h.get("abbreviation", ""))
                        away = _scanner_norm_mlb_team(a.get("abbreviation", ""))
                        if team_abbr not in (home, away):
                            continue
                        side = "Home" if team_abbr == home else "Away"
                        opp = away if side == "Home" else home
                        opp_slot = "away" if side == "Home" else "home"
                        opp_pitcher = teams.get(opp_slot, {}).get("probablePitcher", {}).get("fullName", "")
                        return {
                            "opp": opp or "TBD",
                            "home_team": home,
            ...
            record_debug_error("mlb.edge.hitter.game", e)
        return empty
    ...
    def run_mlb_hitter_fantasy_edge_check(player_name: str, line: float,
        ...
        try:
            ...
            if not is_sane_mlb_pitcher_prop_line(_stat_label, line):
                return None
            basic = _scanner_find_mlb_hitter(player_name)
            team_abbr = _scanner_norm_mlb_team(team or basic.get("team", ""))
            logs = _scanner_get_hitter_logs(player_name, n=14)
            if logs is None or logs.empty or len(logs) < 4:
                return None

            import numpy as _np
            vals = pd.to_numeric(logs["FS"], errors="coerce").dropna().reset_index(drop=True)
            pa_vals = pd.to_numeric(logs["PA"], errors="coerce").dropna().reset_index(drop=True)
            if len(vals) < 4:
                return None

            n = len(vals)
            weights = _np.array([1.6 if i < 3 else 1.15 if i < 6 else 1.0 for i in range(n)])
            ...
            pa_avg = float(pa_vals.mean()) if len(pa_vals) else None
            edge = avg - line
            adj = whr
            ...
            try:
                ...
                expected_fs = per_pa * (pa_avg or 4.0)
            except Exception:
                expected_fs = avg
            proj_prob = _scanner_normal_prob(expected_fs, line, side, sigma=3.6)
            ...
            record_debug_error("mlb.edge.hitter.model", e)
            return None
    ...
    def run_wnba_edge_check(player_name: str, line: float,
        ...
        try:
            ...
            logs = wnba_get_game_logs(player["id"], datetime.now().year, n=24)
            ...
            directional_avg_edge = (
                float(model["avg"]) - line if side == "Over" else line - float(model["avg"])
            )
            return {
                "sport": "WNBA",
                "player": player.get("name") or player_name,
                ...
                "reliability": round(float(model["reliability"]) * 100, 1),
                "avg": round(float(model["avg"]), 1),
                "edge_raw": round(directional_avg_edge, 2),
                "cons": round(float(model["consistency"]) * 100, 1),
                "samples": int(model["sample"]),
                "l3": round(float(model["l3"]), 1),
                ...
                "projection": round(float(model["projection"]), 1),
                "tier": model["tier"],
                "trap_label": trap_label,
                "trap_reasons": flags[:3],
    ...
    if _results:
        ...
        if not _with_edge:
            ...
            for _r in _with_edge:
                ...
                _pa_pill = f"<span class='edge-pill-v55'>PA {_r.get('pa_avg')}</span>" if _r.get("pa_avg") is not None else ""
                _expected_fs_pill = f"<span class='edge-pill-v55'>Exp FS {_r.get('expected_fs')}</span>" if _r.get("expected_fs") is not None else ""
                _opp_profile_pill = (
                ...
                _defense_pill = (
                    ...
                    if _r.get("sport") == "WNBA" and _r.get("defense_label") else ""
                )
                _injury_pill = (
                    f"<span class='edge-pill-v55 {'warn' if _r.get('injury_status') not in ('Active/Unlisted', 'Active') else ''}'>"
                    f"{_r.get('injury_status')}</span>"
...
# Old NBA slate scanner removed. Use Edge for slate-wide scanning.
